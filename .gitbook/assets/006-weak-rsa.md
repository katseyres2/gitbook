# Weak RSA

Download the file : https://app.hackthebox.com/challenges/6
Try to crack the public key with the Python tool [RsaCtfTool](https://github.com/RsaCtfTool/RsaCtfTool?ref=technicalciso.com).

```bash
git clone https://github.com/RsaCtfTool/RsaCtfTool.git
python3 RsaCtfTool.py --publickey id_rsa.pub --private
```

```bash
echo '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----' > id_rsa
```

```bash
openssl pkeyutl -decrypt -inkey id_rsa -in flag.enc -out flag.dec
cat flag.dec
```

The flag is `HTB{s1mpl3_Wi3n3rs_4tt4ck}`.
